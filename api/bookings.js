...
async function sendTelegramBooking(booking) {
  ...
  const services = booking.extra_services ? JSON.parse(JSON.stringify(booking.extra_services)) : [];
  const servicesText = services.length > 0 ? `\n🎉 Доп. услуги: ${services.join(', ')}` : '';

  const text = `📅 НОВОЕ БРОНИРОВАНИЕ #${booking.id}\n\n` +
    `👤 ${booking.customer_name}\n` +
    `📞 ${booking.customer_phone}\n` +
    `📧 ${booking.customer_email || 'не указан'}\n` +
    `🎭 Тип: ${booking.event_type}\n` +
    `📆 Дата: ${booking.event_date}\n` +
    `🕐 Время: ${booking.event_time}\n` +
    `👥 Гостей: ${booking.guests_count}\n` +
    `💰 Смета: ~${booking.total_estimate}₽${servicesText}\n` +
