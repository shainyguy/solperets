...
app.post('/api/bookings', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('table_bookings').insert({
      ...
      occasion:       b.occasion || b.event_occasion || null,
      event_date:     b.event_date,
      event_time:     b.event_time || null,
      guests_count:   b.guests_count,
      table_number:   b.table_number || null,
      package_key:    b.package_key || b.package_name || null,
      extra_services: Array.isArray(b.extra_services)
        ? b.extra_services.map(s => typeof s === 'string' ? s : `${s.name}`).join(', ')
        : (b.extra_services || null),
    ...
    await sendTelegram(
      ...
      `👥 ${data.guests_count} гостей · Стол №${data.table_number || '?'}\n` +
      `📦 ${data.package_key || '—'}\n` +
...
app.post('/api/table-reservations', async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase.from('table_reservations').select('id')
      .eq('table_number', b.table_number).eq('reservation_date', b.reservation_date)
      .eq('reservation_time', b.reservation_time).neq('status', 'cancelled').limit(1);
    ...
    const { data, error } = await supabase.from('table_reservations').insert({
      customer_name: b.customer_name, customer_phone: b.customer_phone,
      table_number: Number(b.table_number), seats: b.seats || null, table_zone: b.table_zone || null,
      guests_count: Number(b.guests_count), reservation_date: b.reservation_date,
      reservation_time: b.reservation_time, occasion: b.occasion || null, comment: b.comment || null, status: 'new'
    }).select().single();
    if (error) throw error;
    await sendTelegram(`🪑 НОВАЯ БРОНЬ СТОЛА #${data.id}\n\n👤 ${data.customer_name}\n📞 ${data.customer_phone}\n🔢 Стол №${data.table_number}${data.table_zone ? ` (${data.table_zone})` : ''}\n👥 Гостей: ${data.guests_count}\n📅 ${data.reservation_date} в ${data.reservation_time}\n🎉 ${data.occasion || 'повод не указан'}\n💬 ${data.comment || 'нет'}`);
    res.status(201).json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// ── /api/reservations → table_reservations ───────────────────────────────────
app.get('/api/reservations', async (req, res) => {
  try {
    const { date, status } = req.query;
    let q = supabase.from('table_reservations').select('*').order('created_at', { ascending: false });
...
app.post('/api/reservations', async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase
      ...
      .limit(1);
    if (existing && existing.length > 0) {
      return res.status(409).json({ error: 'Этот стол уже забронирован на выбранную дату' });
    }
