...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET') {
      ...
      // all=true — для CRM-панели, отдаём все записи без фильтра статуса
      if (all === 'true') {
      ...
      // Для публичной схемы — только активные брони (new + confirmed)
      // cancelled и done НЕ блокируют стол
      let query = supabase.from(TABLE)
        .select('table_number, reservation_date, reservation_time, status')
