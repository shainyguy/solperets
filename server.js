...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({ status: 'ok', db_connected: true, menu_items_count: count });
  } catch (err) {
...
app.post('/api/orders', async (req, res) => {
  try {
    ...
    const items = data.items.map(i => `• ${i.name} ×${i.quantity} — ${i.price * i.quantity}₽`).join('\n');
    await sendTelegram(
...
app.post('/api/reviews', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('reviews').insert({ author_name, rating, text, dish_name: dish_name || null, is_approved: false }).select().single();
    if (error) throw error;
    res.status(201).json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/contacts', async (req, res) => {
  try {
    ...
    if (error) throw error;
    await sendTelegram(`📩 НОВОЕ СООБЩЕНИЕ\n\n👤 ${name}\n📞 ${phone || 'не указан'}\n📧 ${email || 'не указан'}\n💬 ${message}`);
    res.status(201).json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/banquet-services', async (req, res) => {
  try {
    const { data, error } = await supabase.from('banquet_services').select('*').eq('is_active', true).order('sort_order', { ascending: true });
    if (error) throw error;
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    const approved = (reviews.data || []).filter(r => r.is_approved);
    const avgRating = approved.length ? (approved.reduce((s, r) => s + r.rating, 0) / approved.length).toFixed(1) : '0';
    res.json({ total_orders: (orders.data || []).length, new_orders: newOrders, total_revenue: totalRevenue, total_bookings: (bookings.data || []).length, pending_bookings: pendingBookings, pending_reviews: pendingReviews, unread_messages: unreadMessages, avg_rating: avgRating });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
// SPA — все остальные роуты отдают index.html
app.get('*', (req, res) => {
...
app.listen(PORT, () => {
  ...
  console.log(`   Пароль: ${process.env.ADMIN_PASSWORD || 'soliperec2025'}`);
});
