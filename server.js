...
app.post('/api/table-reservations', async (req, res) => {
  try {
    const { table_number, table_zone, customer_name, customer_phone, guests_count, reservation_date, reservation_time, comment } = req.body;
    const { data, error } = await supabase.from('table_reservations').insert({
      table_number, table_zone, customer_name, customer_phone,
      guests_count, reservation_date, reservation_time,
      comment: comment || null, status: 'new'
    }).select().single();
    if (error) throw error;
    await sendTelegram(
      `🪑 БРОНЬ СТОЛА #${data.id}\n\n🔢 Стол №${table_number} (${table_zone})\n👤 ${customer_name}\n📞 ${customer_phone}\n📅 ${reservation_date} в ${reservation_time}\n👥 ${guests_count} гостей\n💬 ${comment || 'Без комментария'}`
    );
    res.status(201).json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/table-bookings', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('bookings_v2').insert({
      ...
      table_number: Number(b.table_number), seats: Number(b.seats) || 4,
      guests_count: Number(b.guests_count), event_date: b.event_date,
      event_time: b.event_time, occasion: b.occasion || null,
      comment: b.comment || null, package: b.package || null, status: 'new'
    }).select().single();
    if (error) throw error;
    await sendTelegram(
      `📅 БРОНИРОВАНИЕ СТОЛА #${data.id}\n\n👤 ${data.customer_name}\n📞 ${data.customer_phone}\n` +
...
app.get('/api/table-reservations', async (req, res) => {
  try {
    const { date } = req.query;
    let query = supabase.from('table_reservations').select('*').neq('status', 'cancelled').order('reservation_date', { ascending: true });
    if (date) query = query.eq('reservation_date', date);
    const { data, error } = await query;
    if (error) throw error;
    res.json(data);
...
app.put('/api/table-reservations', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('table_reservations').update({ status }).eq('id', id).select().single();
    if (error) throw error;
    res.json(data);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.post('/api/reservations', async (req, res) => {
  try {
    const body = req.body;
    const { data, error } = await supabase.from('table_reservations').insert({
      table_number: body.table_number, table_zone: body.table_zone,
      seats_count: body.seats_count, customer_name: body.customer_name,
      customer_phone: body.customer_phone, reservation_date: body.reservation_date,
      reservation_time: body.reservation_time, guests_count: body.guests_count,
      occasion: body.occasion || null, comment: body.comment || null, status: 'new'
    }).select().single();
    if (error) throw error;
    await sendTelegram(
      `🪑 НОВАЯ БРОНЬ СТОЛА #${data.id}\n\n🔢 Стол №${data.table_number} (Зона ${data.table_zone})\n` +
      `👤 ${data.customer_name}\n📞 ${data.customer_phone}\n📅 ${data.reservation_date} в ${data.reservation_time}\n` +
      `👥 Гостей: ${data.guests_count}\n🎭 Повод: ${data.occasion || 'не указан'}\n💬 ${data.comment || 'Без комментария'}`
    );
    res.status(201).json(data);
...
app.post('/api/table-reservations', async (req, res) => {
  try {
    ...
    const { data: existing } = await supabase.from('table_reservations').select('id')
      .eq('table_number', table_number).eq('reserve_date', reserve_date).eq('reserve_time', reserve_time).not('status', 'eq', 'cancelled');
    if (existing && existing.length > 0) return res.status(409).json({ error: 'Этот стол уже забронирован на выбранное время' });
    const { data, error } = await supabase.from('table_reservations').insert({
      table_number, customer_name, customer_phone, guests_count, reserve_date, reserve_time, comment: comment || null, status: 'new'
    }).select().single();
    if (error) throw error;
