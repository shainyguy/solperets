...
import {
  ...
  UtensilsCrossed, Settings, LogOut, Check, X, Sparkles,
  Eye, Trash2, Edit3, Plus, DollarSign,
  RefreshCw, Phone, MapPin, BookOpen, AlertCircle, Image
} from 'lucide-react';
...
export default function AdminPage() {
  ...
  const fetchAll = async () => {
    try {
      const [sR, oR, bR, bbR, rvR, mR, mnR, svR] = await Promise.all([
        fetch('/api/stats'),
        fetch('/api/orders'),
      ...
      if (bbR.ok) setBanquetBookings(await bbR.json());
  ...
  // ── Table Reservations ────────────────────────────────────────────────────
  const updateTableReservation = async (id: number, status: string) => {
  ...
  const deleteTableReservation = async (id: number) => {
    if (!confirm('Отменить бронирование стола?')) return;
    await fetch('/api/table-reservations', {
  ...
  // ── Banquet Bookings ──────────────────────────────────────────────────────
  const updateBanquetBooking = async (id: number, status: string, admin_comment?: string) => {
  ...
  const deleteBanquetBooking = async (id: number) => {
    if (!confirm('Удалить заявку на банкет?')) return;
    await fetch('/api/bookings', {
      method: 'DELETE', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ id }),
    });
  ...
  const navItems = [
    ...
    { key: 'bookings',         label: 'Брони столов',     icon: <Calendar size={18} />,        badge: bookings.filter((b:any)=>b.status==='new').length },
    { key: 'banquet_bookings', label: 'Банкеты',          icon: <Sparkles size={18} />,        badge: banquetBookings.filter((b:any)=>b.status==='new').length },
    ...
    <div className="min-h-screen bg-zinc-950 text-white flex">
      ...
      <div className="flex-1 overflow-auto pb-20 md:pb-0">
        <div className="p-4 md:p-6 max-w-6xl">
          ...
          {section === 'bookings' && (
            <div>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <h1 className="text-2xl font-black">Бронирования столов ({bookings.length})</h1>
                <div className="flex gap-2 text-sm flex-wrap">
                  <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-lg">{bookings.filter((b:any)=>b.status==='confirmed').length} подтв.</span>
                  <span className="px-3 py-1 bg-gray-500/20 text-gray-400 rounded-lg">{bookings.filter((b:any)=>b.status==='cancelled').length} отменено</span>
                </div>
              </div>

              {/* Мини-схема занятых столов */}
              {bookings.filter((b:any) => b.status === 'confirmed').length > 0 && (
                <div className="mb-6 bg-zinc-900 rounded-2xl border border-white/10 p-5">
                  <h3 className="text-sm font-semibold text-white/60 uppercase tracking-wider mb-4">🪑 Активные брони</h3>
                  <div className="flex flex-wrap gap-2">
                    {bookings.filter((b:any) => b.status === 'confirmed').map((b:any) => (
                      <div key={b.id} className="px-3 py-2 rounded-xl border bg-green-900/20 border-green-600/30 text-xs">
                        <p className="font-bold text-green-300">Стол №{b.table_number}</p>
                        <p className="text-white/50">{b.reservation_date} {b.reservation_time} ({b.duration_hours}ч)</p>
                        <p className="text-white/50">{b.customer_name} · {b.guests_count} гостей</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {bookings.map((b:any) => (
                  <div key={b.id} className={`bg-zinc-900 rounded-2xl border p-5 transition-all ${b.status==='confirmed' ? 'border-green-600/30' : b.status==='cancelled' ? 'border-red-900/30 opacity-50' : 'border-white/10'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className="font-bold text-white">#{b.id} — {b.customer_name}</span>
                          <span className={`text-xs px-2 py-0.5 rounded-full text-white ${b.status==='confirmed' ? 'bg-green-600' : b.status==='cancelled' ? 'bg-red-600' : 'bg-gray-600'}`}>
                            {b.status==='confirmed' ? 'Подтверждено' : b.status==='cancelled' ? 'Отменено' : b.status}
                          </span>
                          <span className="text-xs px-2 py-0.5 rounded-full bg-orange-600/20 text-orange-400 font-bold">
                            🪑 Стол №{b.table_number}
                          </span>
                        </div>
                        <div className="flex flex-wrap gap-3 text-xs text-white/50">
                          <span>📞 {b.customer_phone}</span>
                          <span>📅 {b.reservation_date} в {b.reservation_time}</span>
                          <span>⏱ {b.duration_hours} ч.</span>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {b.status !== 'cancelled' && (
                        <button onClick={() => updateTableReservation(b.id, 'cancelled')}
                          className="flex items-center gap-1.5 px-3 py-2 bg-red-700/30 hover:bg-red-700 text-red-400 hover:text-white text-sm rounded-lg transition-colors">
                          🔓 Отменить
                        </button>
                      )}
                      <button onClick={() => deleteTableReservation(b.id)}
                        className="flex items-center gap-1.5 px-3 py-2 bg-white/5 hover:bg-red-700 text-white/40 hover:text-white text-sm rounded-lg transition-colors ml-auto">
                ...
                {bookings.length === 0 && (
                  <div className="text-center py-16 text-white/30">
                    ...
                    <p>Бронирований столов пока нет</p>
                  </div>
                )}
              </div>
            </div>
          ...
          {/* BANQUET BOOKINGS */}
          {section === 'banquet_bookings' && (
            <div>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <h1 className="text-2xl font-black">Банкеты и мероприятия ({banquetBookings.length})</h1>
                <div className="flex gap-2 text-sm flex-wrap">
                  <span className="px-3 py-1 bg-yellow-500/20 text-yellow-400 rounded-lg">{banquetBookings.filter((b:any)=>b.status==='new').length} новых</span>
                  <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-lg">{banquetBookings.filter((b:any)=>b.status==='confirmed').length} подтв.</span>
                </div>
              </div>
              <div className="space-y-4">
                {banquetBookings.map((b:any) => (
                  <div key={b.id} className={`bg-zinc-900 rounded-2xl border p-5 ${b.status==='new' ? 'border-yellow-600/40' : b.status==='confirmed' ? 'border-green-600/30' : b.status==='cancelled' ? 'border-red-900/30 opacity-60' : 'border-white/10'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                          <span className="font-bold text-white">#{b.id} — {b.customer_name}</span>
                          <span className={`text-xs px-2 py-0.5 rounded-full text-white ${BOOKING_STATUS[b.status as keyof typeof BOOKING_STATUS]?.color||'bg-gray-500'}`}>
                            {BOOKING_STATUS[b.status as keyof typeof BOOKING_STATUS]?.label || b.status}
                          </span>
                          {b.package_name && <span className="text-xs px-2 py-0.5 rounded-full bg-purple-600/20 text-purple-300">📦 {b.package_name}</span>}
                        </div>
                        <div className="flex flex-wrap gap-3 text-xs text-white/50">
                          <span>📞 {b.customer_phone}</span>
                    ...
                    {b.extra_services?.length > 0 && (
                      <div className="mb-3 p-3 bg-white/5 rounded-xl text-xs text-white/50">
                        🎉 Доп. услуги: {b.extra_services.join(', ')}
                      </div>
                    )}
                    {b.comment && <p className="text-white/50 text-sm mb-3">💬 {b.comment}</p>}
                    {b.admin_comment && <p className="text-orange-400/70 text-sm mb-3">📝 {b.admin_comment}</p>}

                    <div className="flex flex-wrap gap-2 mb-3">
                      {Object.entries(BOOKING_STATUS).map(([st, {label, color}]) => (
                        <button key={st} onClick={() => updateBanquetBooking(b.id, st)}
                    ...
                    <div className="mb-3">
                      <input placeholder="Заметка администратора..."
                        defaultValue={b.admin_comment || ''}
                        onBlur={e => { if (e.target.value !== (b.admin_comment||'')) updateBanquetBooking(b.id, b.status, e.target.value); }}
                        className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-white/20 focus:outline-none focus:border-orange-500"
                      />
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      <button onClick={() => deleteBanquetBooking(b.id)} className="flex items-center gap-1.5 px-3 py-2 bg-white/5 hover:bg-red-700 text-white/40 hover:text-white text-sm rounded-lg transition-colors ml-auto">
                        <Trash2 size={14}/> Удалить
                      </button>
                    </div>
                  </div>
                ...
                {banquetBookings.length === 0 && (
                  <div className="text-center py-16 text-white/30">
                    ...
                    <p>Заявок на банкеты пока нет</p>
                  </div>
                )}
              </div>
            </div>
