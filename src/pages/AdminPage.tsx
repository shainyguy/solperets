...
import {
  ...
  Eye, Trash2, Edit3, Plus, DollarSign, Users,
...
export default function AdminPage() {
  ...
  const updateTableRes = async (id: number, status: string) => {
    await api('/api/table-bookings', { method: 'PUT', body: JSON.stringify({ id, status }) });
    fetchAll();
  };

  const deleteTableRes = async (id: number) => {
    await api('/api/table-bookings', { method: 'DELETE', body: JSON.stringify({ id }) });
    fetchAll();
  };
  ...
  const nav = [
    ...
    { key: 'tables',     label: 'Брони столов',  icon: <Users size={18} />,           badge: null },
    { key: 'tables',     label: 'Брони столов',  icon: <Users size={18} />,           badge: null },
    ...
    <div className="min-h-screen bg-zinc-950 text-white flex">
      ...
      <div className="flex-1 overflow-auto pb-20 md:pb-0">
        <div className="p-4 md:p-6 max-w-6xl">
                    ...
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-3 text-xs text-white/50">
                          ...
                          <span>📅 {t.event_date} в {t.event_time}</span>
                          <span>👥 {t.guests_count} гостей</span>
                          <span>🪑 {t.seats} мест за столом</span>
                          <span>📍 Зона: {t.zone}</span>
                        </div>
                      </div>
                    </div>
          ...
          {/* ── БРОНИ СТОЛОВ ── */}
          {section === 'tables' && <TableReservationsSection />}
...
// ─── Компонент: Брони столов ─────────────────────────────────────────────────
const TABLE_STATUS: Record<string, { label: string; color: string }> = {
  new:       { label: 'Новая',      color: 'bg-blue-500' },
  confirmed: { label: 'Подтверждена', color: 'bg-green-500' },
  done:      { label: 'Завершена',  color: 'bg-gray-500' },
  cancelled: { label: 'Отменена',   color: 'bg-red-500' },
};
...
function TableReservationsSection() {
  ...
  const load = async () => {
    setLoading(true);
    const url = filterDate ? `/api/table-bookings?date=${filterDate}` : '/api/table-bookings';
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
        <h1 className="text-2xl font-black">Брони столов ({reservations.length})</h1>
        <div className="flex items-center gap-3 bg-zinc-900 border border-white/10 rounded-xl px-4 py-2">
          <span className="text-white/50 text-sm">Дата:</span>
          <input type="date" value={filterDate} onChange={e => setFilterDate(e.target.value)}
            className="bg-transparent text-white text-sm focus:outline-none" />
          {filterDate && <button onClick={() => setFilterDate('')} className="text-white/30 hover:text-white text-xs">✕ сброс</button>}
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-white/40">Броней нет</div>
      ) : (
        <div className="space-y-4">
          {reservations.map(r => (
            <div key={r.id} className={`bg-zinc-900 rounded-2xl border p-5 ${r.status === 'cancelled' ? 'border-red-500/20 opacity-60' : 'border-white/10'}`}>
              <div className="flex flex-wrap items-start justify-between gap-4 mb-3">
                <div>
                  <div className="flex items-center gap-3 mb-1">
                    <h3 className="font-bold text-white">#{r.id} — Стол №{r.table_number}</h3>
                    <span className={`text-xs px-2 py-1 rounded-full text-white ${TABLE_STATUS[r.status]?.color || 'bg-gray-500'}`}>
                      {TABLE_STATUS[r.status]?.label || r.status}
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-3 text-sm text-white/50">
                    <span>👤 {r.customer_name}</span>
                    <a href={`tel:${r.customer_phone}`} className="text-orange-400 hover:underline">📞 {r.customer_phone}</a>
                    <span>📅 {r.reservation_date} в {r.reservation_time}</span>
                    <span>👥 {r.guests_count} гостей</span>
                    <span>🪑 {r.seats} мест за столом</span>
                    {r.occasion && <span>🎉 {r.occasion}</span>}
                  </div>
                  {r.comment && <p className="text-white/40 text-sm mt-1">💬 {r.comment}</p>}
                </div>
              </div>
              <div className="flex flex-wrap gap-2">
                {Object.entries(TABLE_STATUS).map(([st, { label, color }]) => (
                  <button key={st} onClick={() => updateStatus(r.id, st)}
                    disabled={r.status === st}
                    className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                      r.status === st ? `${color} text-white` : 'bg-white/10 text-white/60 hover:bg-white/20'
                    }`}>{label}</button>
                ))}
                <a href={`tel:${r.customer_phone}`}
                  className="flex items-center gap-1 px-3 py-1.5 bg-green-700 hover:bg-green-600 text-white text-xs rounded-lg transition-colors">
                  📞 Позвонить
                </a>
              </div>
            </div>
