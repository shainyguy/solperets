...
async function sendTelegramBooking(booking) {
  ...
  const services = (booking.extra_services || []).join(', ') || '—';
  const tableStr = booking.table_number ? `Стол №${booking.table_number} (Зона ${booking.table_zone || '?'})` : 'Не выбран';
  ...
  const text =
    ...
    `🎭 Повод: ${booking.event_type}\n` +
    `📆 Дата: ${booking.event_date} в ${booking.event_time}\n` +
    `👥 Гостей: ${booking.guests_count}\n` +
    `🪑 ${tableStr}\n` +
    `🎉 Пакет/услуги: ${services}\n` +
    `💰 ~${booking.total_estimate?.toLocaleString('ru-RU') || 0}₽\n` +
  ...
  try {
    ...
    if (req.method === 'POST') {
      ...
      const { data, error } = await supabase.from('bookings_v2').insert({
        ...
        table_zone:      body.table_zone      || null,
