...
import {
  ...
  Trash2, Edit3, Plus, RefreshCw, Phone, BookOpen, MapPin,
...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const [o, b, r, m, mi, sv, tr] = await Promise.all([
  ...
  const updateTableRes = async (id: number, upd: any) => {
    // Когда статус меняется на cancelled/done — стол автоматически освобождается
    // потому что GET /api/table-reservations фильтрует только new+confirmed
    await sb.from('table_reservations').update(upd).eq('id', id);
    load();
  };

  const deleteTableRes = async (id: number) => {
    await sb.from('table_reservations').delete().eq('id', id);
    load();
  };
  ...
  const navItems = [
    ...
    { key: 'tables',    label: 'Брони столов',  icon: <MapPin size={18} />,          badge: tableRes.filter((t: any) => t.status === 'new').length },
      ...
      <main className="flex-1 overflow-auto pb-20 md:pb-0">
        <div className="p-4 md:p-6 max-w-6xl mx-auto">
          ...
          {section === 'tables' && (
            <div>
              <div className="flex items-center justify-between mb-6">
                <h1 className="text-2xl font-black">Брони столов <span className="text-white/30 text-lg">({tableRes.length})</span></h1>
                <button onClick={load} className="flex items-center gap-2 px-3 py-2 bg-white/10 hover:bg-white/20 rounded-xl text-sm transition-colors"><RefreshCw size={14} /> Обновить</button>
              </div>
              <div className="mb-4 p-3 bg-blue-900/20 border border-blue-500/20 rounded-xl text-blue-300 text-sm">
                💡 При смене статуса на <strong>«Отменена»</strong> или <strong>«Завершена»</strong> стол автоматически освобождается на схеме зала
              </div>
              {tableRes.length === 0 && !loading && <p className="text-white/30 text-center py-12">Броней столов пока нет</p>}
              <div className="space-y-3">
                {tableRes.map((t: any) => (
                  <div key={t.id} className="bg-zinc-900 rounded-2xl border border-white/10 p-4">
                    <div className="flex flex-wrap items-start gap-3 justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs px-2 py-1 rounded-full text-white font-medium ${
                            t.status === 'new' ? 'bg-blue-600' :
                            t.status === 'confirmed' ? 'bg-green-600' :
                            t.status === 'cancelled' ? 'bg-red-600' : 'bg-gray-600'
                          }`}>
                            {t.status === 'new' ? 'Новая' : t.status === 'confirmed' ? 'Подтверждена' : t.status === 'cancelled' ? 'Отменена' : 'Завершена'}
                          </span>
                          <span className="text-white font-bold">#{t.id} — Стол №{t.table_number}</span>
                          {t.zone && <span className="text-white/40 text-xs">Зона {t.zone}</span>}
                        </div>
                        <div className="flex flex-wrap gap-3 text-sm text-white/50">
                          <span>👤 {t.customer_name}</span>
                          <a href={`tel:${t.customer_phone}`} className="text-orange-400 hover:underline">📞 {t.customer_phone}</a>
                          <span>📅 {t.reservation_date} в {t.reservation_time}</span>
                          <span>👥 {t.guests_count} гостей</span>
                        </div>
                        {t.comment && <p className="text-white/40 text-xs mt-1">💬 {t.comment}</p>}
                      </div>
                    </div>
                    {/* Смена статуса */}
                    <div className="flex flex-wrap gap-2">
                      {[
                        { key: 'new',       label: 'Новая',        color: 'bg-blue-600' },
                        { key: 'confirmed', label: '✓ Подтвердить', color: 'bg-green-600' },
                        { key: 'cancelled', label: '✗ Отменить',    color: 'bg-red-600' },
                        { key: 'done',      label: '★ Завершена',   color: 'bg-gray-600' },
                      ].map(s => (
                        <button key={s.key}
                          onClick={() => updateTableRes(t.id, { status: s.key })}
                          className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
                            t.status === s.key ? `${s.color} text-white` : 'bg-white/10 text-white/60 hover:bg-white/20'
                          }`}>
                          {s.label}
                        </button>
                      ))}
                      <button onClick={() => deleteTableRes(t.id)}
                        className="px-3 py-1.5 rounded-lg text-xs font-medium bg-red-900/30 text-red-400 hover:bg-red-700 hover:text-white transition-colors ml-auto">
