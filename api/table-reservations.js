...
export default async function handler(req, res) {
  ...
  try {
    // GET — получить брони (опционально фильтр по дате)
    if (req.method === 'GET') {
    ...
    if (req.method === 'POST') {
      const body = req.body || {};
      const { table_number, zone, customer_name, customer_phone, guests_count, reservation_date, reservation_time, comment } = body;

      if (!table_number || !customer_name || !customer_phone || !guests_count || !reservation_date || !reservation_time)
        return res.status(400).json({ error: 'Заполните все обязательные поля' });

      // Проверка что стол не занят в это время
