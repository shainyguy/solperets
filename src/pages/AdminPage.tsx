...
export default function AdminPage() {
  ...
  const fetchAll = async () => {
    try {
      const [sR, oR, bR, rvR, mR, mnR, svR] = await Promise.all([
        fetch('/api/stats'),
        fetch('/api/orders'),
        fetch('/api/bookings'),  // теперь bookings_v2 через api/bookings.js
        fetch('/api/reviews'),
        fetch('/api/contacts'),
        fetch('/api/menu'),
  ...
  // ── Bookings ──────────────────────────────────────────────────────────────
  const updateBooking = async (id: number, status: string, admin_comment?: string) => {
          ...
          {section === 'bookings' && (
            <div>
              <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
                <h1 className="text-2xl font-black">Бронирования ({bookings.length})</h1>
                <div className="flex gap-2 text-sm">
                  <span className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-lg">{bookings.filter(b=>b.status==='new').length} новых</span>
                  <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-lg">{bookings.filter(b=>b.status==='confirmed').length} подтв.</span>
                </div>
              </div>

              {/* Мини-карта занятых столов */}
              {bookings.filter(b => b.table_number && b.status !== 'cancelled').length > 0 && (
                <div className="mb-6 bg-zinc-900 rounded-2xl border border-white/10 p-5">
                  <h3 className="text-sm font-semibold text-white/60 uppercase tracking-wider mb-4">🗺️ Занятые столы</h3>
                  <div className="flex flex-wrap gap-2">
                    {bookings.filter(b => b.table_number && b.status !== 'cancelled').map(b => (
                      <div key={b.id} className={`px-3 py-2 rounded-xl border text-xs ${b.status === 'new' ? 'bg-yellow-900/30 border-yellow-600/40 text-yellow-300' : b.status === 'confirmed' ? 'bg-red-900/30 border-red-600/40 text-red-300' : 'bg-zinc-800 border-white/10 text-white/50'}`}>
                        <p className="font-bold">Стол №{b.table_number}</p>
                        <p className="opacity-70">{b.event_date} {b.event_time}</p>
                        <p className="opacity-70">{b.customer_name}</p>
                        <p className="opacity-60">{b.guests_count} гостей</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="space-y-4">
                {bookings.map(b => (
                  <div key={b.id} className={`bg-zinc-900 rounded-2xl border p-5 transition-all ${b.status==='new' ? 'border-yellow-600/40' : b.status==='confirmed' ? 'border-green-600/30' : b.status==='cancelled' ? 'border-red-900/30 opacity-60' : 'border-white/10'}`}>
                    <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                      <div>
                        <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {b.event_type && <p className="text-white/60 text-sm mb-2">📦 Пакет: {b.event_type}</p>}
                    ...
                    {b.admin_comment && <p className="text-orange-400/70 text-sm mb-3">📝 Заметка: {b.admin_comment}</p>}
                    ...
                    <div className="mb-3">
                      <input
                        ...
                        onBlur={e => { if (e.target.value !== (b.admin_comment||'')) updateBooking(b.id, b.status, e.target.value); }}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {b.status !== 'cancelled' && b.table_number && (
                        <button onClick={() => updateBooking(b.id, 'cancelled')}
                          ...
                          🔓 Освободить стол №{b.table_number}
                        </button>
                      )}
